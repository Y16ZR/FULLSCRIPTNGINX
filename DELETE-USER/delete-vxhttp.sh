...
NUMBER_OF_CLIENTS=$(grep -c -E "^#vmsxhttp " "/usr/local/etc/xray/config.json")
	...
	clear
	echo " Delete User Xray Vmess & Vless XHTTP"
...
sed -i "/^#vmsxhttp $user $exp $harini $uuid/,/^},{/d" /usr/local/etc/xray/config.json
sed -i "/^#vlsxhttp $user $exp $harini $uuid/,/^},{/d" /usr/local/etc/xray/config.json
rm -f /usr/local/etc/xray/$user-vmess-xhttp.json
systemctl restart xray
clear
echo " XRAY VMESS & VLESS XHTTP Account Deleted Successfully"
echo " =========================="
