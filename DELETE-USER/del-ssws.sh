...
CLIENT_NAME=$(grep -E "^### " "/usr/local/etc/xray/akunssws.conf" | cut -d ' ' -f 2-3 | sed -n "${CLIENT_NUMBER}"p)
...
sed -i "/^### $user $exp $harini $uuid/d" /usr/local/etc/xray/akunssws.conf
sed -i "/^#ssws $user $exp $harini $uuid/,/^},{/d" /usr/local/etc/xray/config.json
