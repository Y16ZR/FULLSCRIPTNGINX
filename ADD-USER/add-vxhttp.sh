...
echo -e   "  \e[$line═══════════════════════════════════════════════════════\e[m"
echo -e   "  \e[$back_text       \e[0;30m[\e[$box CREATE USER XRAY VMESS & VLESS XHTTP\e[0;30m ]\e[0m\e[$back_text        \e[m"
echo -e   "  \e[$line═══════════════════════════════════════════════════════\e[m"
...
vlesslink1="vless://${uuid}@${sts}${domain}:${none}?path=${patchvless}&security=tls&encryption=none&host=${domain}&type=xhttp&sni=${stn}#${user}"
...
vlesslink1="vless://${uuid}@${sts}:${none}?path=${patchvless}&security=tls&encryption=none&host=${domain}&type=xhttp&sni=${domain}#${user}"
systemctl restart xray
clear
echo -e "\e[$line════════[ACC XRAY VMESS & VLESS XHTTP]═══════\e[m"
echo -e "Remarks        : ${user}"
